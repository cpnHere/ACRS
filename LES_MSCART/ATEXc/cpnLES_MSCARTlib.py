...
#        dV=f['dV'][:]
    ...
    def savePOLCARThdf5(self,out_name,dpath=None,pc_format=None,action=None):
        ...
        PC=f.create_dataset('File_stamp/cc3D',data=np.array([self.cc3D]))
        PC.attrs['units']='None'
        PC.attrs['long_name']='Column_by_column_or_3D'

        PC=f.create_dataset('File_stamp/dset',data=np.array([self.dset]))
        PC.attrs['units']='None'
        PC.attrs['long_name']='POLCARTdset_obj_name'

        PC=f.create_dataset('File_stamp/fname',data=np.array([self.fname]))
        PC.attrs['units']='None'
        PC.attrs['long_name']='Origina_data_file_name'

        PC=f.create_dataset('File_stamp/nmldpath',data=np.array([self.nmldpath]))
        PC.attrs['units']='None'
    ...
    def readPOLCARThdf5(self,filename,dpath=None):
        if dpath==None:
            ...
            self.hdfpath=dpath
        self.cc3D=f['File_stamp/cc3D'][0]
        self.dset=f['File_stamp/dset'][0]
        self.fname=f['File_stamp/fname'][0]
        self.nmldpath=f['File_stamp/nmldpath'][0]
...
class LES_case(object):
    ...
    def __init__(self,MSCARThdf,base_dir,result_dir=None,RT1Dname=None,band=None):
        ...
        self.RT_field=LES_field(self.RT.fname.split('_MSCART',1)[0]+'.nc',dpath=base_dir)
...
class DHARMA_onmp(object):
    ...
    def setupMieOut_crude(self,mie_name):
        ...
        '''
        case=self
        cloud_mie=MieSet(mie_name)
        cloud_mie.readMie()
        extp3d=np.einsum('ij,i,iklm->jiklm',cloud_mie.qe,case.DHARMA.r_drops**2,case.DHARMA.dN_drops)*np.pi*1e-6#m^-1
        s1=case.DHARMA.dN_drops.shape    
        s2=cloud_mie.wvl.size
        omgp3d=np.ones((s2,s1[0],s1[1],s1[2],s1[3]),dtype=float)
        omgp3d=np.einsum('ij,jiklm->jiklm',cloud_mie.alb,omgp3d)
        P11=cloud_mie.P11.swapaxes(0,1)
        P33=cloud_mie.P33.swapaxes(0,1)
        P12=cloud_mie.P12.swapaxes(0,1)
        P34=cloud_mie.P34.swapaxes(0,1)    
        return extp3d,omgp3d,P11,P33,P12,P34,cloud_mie
        
    def get_pDefr_l(self,name):
        '''
        Predefined radii bin edges for each LES case
        ----------------------------------------------
        An example of finding bind edges
    ...
    def setupMieOut(self,mie_name,lesBinEd=None,lesCname=None,mie_path=None):
        ...
        al=c_mie.alb
        r=c_mie.d/2
        P11,P33,P12,P34=c_mie.P11,c_mie.P33,c_mie.P12,c_mie.P34
        ...
        aP11=np.zeros((qe.shape[1],r_l.size-1,c_mie.ang.size),dtype=float)
        ...
        return extp3d,omgp3d,aP11,aP33,aP12,aP34,c_mie
    ...
    def find_re(self,mie_name,lesBinEd=None,lesCname=None,mie_path=None):
        '''
        This effective radius is a kind of meaning-less.!!! Don't use without rederiving the theory.
        --------------------------------------------------------------------------------------------
        To find effective radii re. Must give Mie code output file.
        mie_name: String name of the Mie output file without *.nc
        --------------------------------------------------------------------------------------------
        lesBinEd: Array of the LES radii bin edges
            Indices of r that are nearest to the bind edge values in DHARMA.rbound_drops
        ...
        for i in np.arange(0,r_l.size-1,1):
            ...
            rt=r_l[i+1]
            qe_avg[:,i]=np.einsum('ij,i->j'        ,qe[lt:rt,:],r[lt:rt]**2)/np.sum(r[lt:rt]**2)
        re=np.einsum('ji,i,iklm->jiklm',qe_avg,self.DHARMA.r_drops**3,self.DHARMA.dN_drops)/np.einsum('ji,i,iklm->jiklm',qe_avg,self.DHARMA.r_drops**2,self.DHARMA.dN_drops)#um
        return re
