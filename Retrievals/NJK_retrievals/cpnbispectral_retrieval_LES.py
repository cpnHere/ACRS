...
if __name__=='__main__':
    ...
    #New runs
    #'''
    NJK_ret3D=doNJK_LES(DYC0p860_sza,DYC2p13_sza,sza,vza,check_DB=False,pro_type='DISORT')
    NJK_ret1D=doNJK_LES(DYC0p860_sza,DYC2p13_sza,sza,vza,RTdim='1D',check_DB=False,pro_type='DISORT')
    
    cpn.save_obj(NJK_ret3D,sdir+NJK_ret3D.NJK_case_name)
    cpn.save_obj(NJK_ret1D,sdir+NJK_ret1D.NJK_case_name)
    #Saving directly as hdf5 files
    NJK_ret3D.save_to_hdf5(NJK_ret3D.NJK_case_name,path=sdir)
    NJK_ret1D.save_to_hdf5(NJK_ret1D.NJK_case_name,path=sdir)
    #'''
    #Load previous runs
    '''
